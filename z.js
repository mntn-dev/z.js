    ...
    var z = {
        ...
        rc4: function(d, p) {
            ...
            var a = [],
                j = 0,
                R = "";
            for (i = 0; 256 > i; i++) a[i] = i;
            for (i = 0; i < 256; i++) {
                j = (j + a[i] + p.toString().charCodeAt(i % p.length)) % 256;
                Z =
                    a[i];
                a[i] = a[j];
                a[j] = Z;
            ...
            for (l = 0; l < d.length; l++) {
                ...
                a[i] = a[j];
                a[j] = Z;
                R += String.fromCharCode(d.charCodeAt(l) ^ a[(a[i] + a[j]) % 256]);
            }
            return (R);
        },
        js: function(d, p, r) {
            ...
            if (RegExp(C).test(d)) { //restoring
                if (RegExp(B).test(d)) { //parts-mode
                    d = d.replace(RegExp(B, 'g'), '').replace(/([\s\S])([\u200c\u200d]+)/g, function(_1, _2) {
                        t = _2;
                        return (z.js.apply(!1, [_1, (p || !1), (r === !!1) ? !!1 : !1]) || (t = !1));
                    }).
                    replace(RegExp('\\' + t, 'g'), '');
                    return (!t ? !1 : d);
                }
                d = this.rc4(d.replace(RegExp('[^' + C + D + ']', 'g'), '').replace(/.{8}/g, function(_1) {
                    return (
                        String.fromCharCode(parseInt(_1.replace(RegExp(C, 'g'), '0').replace(RegExp(D, 'g'), '1'), 2)));
                }), p);
                return (RegExp(A).test(d) ? d.slice(8) : ((r === !!1) ? d : !1));
            }
            if (r && r[1]) { //hiding: parts-mode
                t = '';
                if (r[1].charCodeAt(0) < 48) t = '\\';
                if ((d.match(RegExp(t + r[1], 'g')) || [1]).length % 2 != 0) return (!1);
                return (d.replace(RegExp(t + r[1] + '{2}', 'g'), '').replace(RegExp(t + r[1] + '[\\s\\S]+?' + t + r[1], 'g'),
                    function(_1) {
                        return (B + z.js.apply(!1, [_1.slice(1, -1), (p || !1), r[0]]));
                    }));
            }
            ...
            if (r) r = d.replace(/<.*?>/g, '').replace(/[^ \t\n]/g, ((r === !!1) ? '\u2589' : r.toString()[0]))
                .replace(/ {2,}/g, ' ');
            d = d.replace(/[\u0100-\uffff]/g, function(_1) {
                return ('&#' + _1.charCodeAt(0) + ';');
            });
            d = this.rc4(A + d, p).replace(/[\s\S]/g, function(_1) {
                t = _1.charCodeAt(0).toString(2);
                return (t[13] ? _1 :
